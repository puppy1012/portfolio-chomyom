package com.chomyom.portfolio.domain

import com.chomyom.portfolio.domain.constant.SkillType
import com.chomyom.portfolio.domain.entity.*
import com.chomyom.portfolio.domain.repository.*
...
@Component
@Profile(value = ["default"]) // 기본 프로파일에서만 활성화
class DataInitializer(
    ...
    val log = LoggerFactory.getLogger(DataInitializer::class.java) // 로깅 설정

    @PostConstruct
    fun initializeData() {
        log.info("스프링이 실행되었습니다. 테스트 데이터를 초기화합니다.") // 데이터 초기화 로그 출력

        // Achievement(업적) 데이터 초기화
        ...
        achievementRepository.saveAll(achievements) // 업적 데이터 저장

        // Introduction(소개) 데이터 초기화
        ...
        introductionRepository.saveAll(introductions) // 소개 데이터 저장
        ...
        linkRepository.saveAll(links) // 링크 데이터 저장

        // Experience(경험) 데이터 초기화
        ...
        experienceRepository.saveAll(mutableListOf(experience1, experience2)) // 경험 데이터 저장
        ...
        skillRepository.saveAll(
            mutableListOf(java, kotlin, python, spring, django, mysql, redis, kafka)
        ) // 기술 스택 저장

        // Project(프로젝트) 데이터 초기화
        ...
        projectRepository.saveAll(mutableListOf(project1, project2)) // 프로젝트 데이터 저장
    }
}
